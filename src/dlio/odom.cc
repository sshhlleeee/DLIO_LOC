...
dlio::OdomNode::OdomNode() : Node("dlio_odom_node") {
  ...
  this->deskew_size = 0;
  this->initial_pose_received_ = false;
  ...
  this->initial_pose_cb_group_ = this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  auto initial_pose_sub_opt = rclcpp::SubscriptionOptions();
  initial_pose_sub_opt.callback_group = this->initial_pose_cb_group_;
  this->initial_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>("/initialpose", 10,
      std::bind(&dlio::OdomNode::callbackInitialPose, this, std::placeholders::_1), initial_pose_sub_opt);
  ...
  this->map_pub_      = this->create_publisher<sensor_msgs::msg::PointCloud2>("map", map_qos_profile);
  ...
  this->loaded_map_pcd_ = std::make_shared<pcl::PointCloud<PointType>>();
  ...
  //
  // convex, concave hull setting for submap building
  //
  ...
  // small_gicp
  //
  // this->gicp.reduction.num_threads = this->num_threads_;
  // this->gicp.rejector.max_dist_sq = this->gicp_max_corr_dist_ * this->gicp_max_corr_dist_;
  // this->gicp.criteria.rotation_eps = this->gicp_rotation_ep_;
  // this->gicp.criteria.translation_eps = this->gicp_transformation_ep_;
  // this->gicp.optimizer.max_iterations = this->gicp_max_iter_;
  ...
  // gicp settings
  //
  if (this->gicp_implementation_ == "small_gicp") {
    // small_gicp 초기화
    if (this->sgicp_use_robust_) {
      this->cauchy_sgicp.reduction.num_threads = this->num_threads_;
      this->cauchy_sgicp.rejector.max_dist_sq = this->gicp_max_corr_dist_ * this->gicp_max_corr_dist_;
      this->cauchy_sgicp.criteria.rotation_eps = this->gicp_rotation_ep_;
      this->cauchy_sgicp.criteria.translation_eps = this->gicp_transformation_ep_;
      this->cauchy_sgicp.optimizer.max_iterations = this->gicp_max_iter_;
    } else {
      this->sgicp.reduction.num_threads = this->num_threads_;
      this->sgicp.rejector.max_dist_sq = this->gicp_max_corr_dist_ * this->gicp_max_corr_dist_;
      this->sgicp.criteria.rotation_eps = this->gicp_rotation_ep_;
      this->sgicp.criteria.translation_eps = this->gicp_transformation_ep_;
      this->sgicp.optimizer.max_iterations = this->gicp_max_iter_;
    }
    RCLCPP_INFO(this->get_logger(), "Using small_gicp (robust_kernel: %s)", 
                this->sgicp_use_robust_ ? "true" : "false");
  } else {
    // nano_gicp 초기화
    pcl::Registration<PointType, PointType>::KdTreeReciprocalPtr temp;
    this->ngicp.setSearchMethodSource(temp, true);
    this->ngicp.setSearchMethodTarget(temp, true);
    this->ngicp_temp.setSearchMethodSource(temp, true);
    this->ngicp_temp.setSearchMethodTarget(temp, true);
    
    this->ngicp.setCorrespondenceRandomness(this->gicp_k_correspondences_);
    this->ngicp.setMaxCorrespondenceDistance(this->gicp_max_corr_dist_);
    this->ngicp.setMaximumIterations(this->gicp_max_iter_);
    this->ngicp.setTransformationEpsilon(this->gicp_transformation_ep_);
    this->ngicp.setRotationEpsilon(this->gicp_rotation_ep_);
    this->ngicp.setInitialLambdaFactor(this->gicp_init_lambda_factor_);
    
    this->ngicp_temp.setCorrespondenceRandomness(this->gicp_k_correspondences_);
    this->ngicp_temp.setMaxCorrespondenceDistance(this->gicp_max_corr_dist_);
    this->ngicp_temp.setMaximumIterations(this->gicp_max_iter_);
    this->ngicp_temp.setTransformationEpsilon(this->gicp_transformation_ep_);
    this->ngicp_temp.setRotationEpsilon(this->gicp_rotation_ep_);
    this->ngicp_temp.setInitialLambdaFactor(this->gicp_init_lambda_factor_);
    
    RCLCPP_INFO(this->get_logger(), "Using nano_gicp");
  }


  //
  // geometrical observer settings
  //
  ...
  this->trajectory.set_capacity(5000); 
  this->comp_times.set_capacity(1000);
  this->imu_rates.set_capacity(1000);
  this->lidar_rates.set_capacity(1000);
  this->global_comp_time_sum = 0.;
  this->global_avg_comp_time = 0.;
  this->global_comp_frame_count = 0;
  ...
  // Initialize debug output rate control (allow immediate first output)
  this->last_debug_time = std::chrono::steady_clock::now() - std::chrono::seconds(2);
  ...
  // localization mode일 경우 맵 로딩
  if (this->localization_mode_) {
    RCLCPP_INFO(this->get_logger(), "Localization Mode Enabled.");
    this->loadMap(); // loadMap 함수는 여기서 호출
    
    // If auto initial pose is enabled, set it immediately using the same logic as callbackInitialPose
    if (this->use_auto_initial_pose_) {
      // Use the shared internal function to ensure consistent behavior
      this->setInitialPoseInternal(
        this->auto_initial_pose_x_,
        this->auto_initial_pose_y_,
        this->auto_initial_pose_z_,
        this->auto_initial_pose_qx_,
        this->auto_initial_pose_qy_,
        this->auto_initial_pose_qz_,
        this->auto_initial_pose_qw_
      );
      
      std::cout << "| Auto initial pose set:                                            |" << std::endl;
      std::cout << "|   Position: x=" << std::fixed << std::setprecision(3) 
                << this->auto_initial_pose_x_ << ", y=" << this->auto_initial_pose_y_ 
                << ", z=" << this->auto_initial_pose_z_ << std::setw(27) << "|" << std::endl;
      std::cout << "|   Orientation: qx=" << std::fixed << std::setprecision(3)
                << this->auto_initial_pose_qx_ << ", qy=" << this->auto_initial_pose_qy_ << std::setw(22) << "|" << std::endl;
      std::cout << "|                qz=" << std::fixed << std::setprecision(3)
                << this->auto_initial_pose_qz_ << ", qw=" << this->auto_initial_pose_qw_ << std::setw(22) << "|" << std::endl;
    } else {
...
void dlio::OdomNode::getParams() {
  ...
  dlio::declare_param(this, "localization_mode", this->localization_mode_, false);
  dlio::declare_param(this, "map_path", this->map_path_, std::string(""));
  
  // Get auto initial pose parameters (optional)
  dlio::declare_param(this, "auto_initial_pose/enabled", this->use_auto_initial_pose_, false);
  dlio::declare_param(this, "auto_initial_pose/position/x", this->auto_initial_pose_x_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/position/y", this->auto_initial_pose_y_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/position/z", this->auto_initial_pose_z_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/orientation/x", this->auto_initial_pose_qx_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/orientation/y", this->auto_initial_pose_qy_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/orientation/z", this->auto_initial_pose_qz_, 0.0);
  dlio::declare_param(this, "auto_initial_pose/orientation/w", this->auto_initial_pose_qw_, 1.0);
  ...
  dlio::declare_param(this, "odom/gicp/implementation", this->gicp_implementation_, std::string("small_gicp"));
  dlio::declare_param(this, "odom/gicp/useRobustKernel", this->sgicp_use_robust_, true);
...
void dlio::OdomNode::publishPose() {
  // Do not publish anything until initialized
  if (!this->dlio_initialized) {
    return;
  }
  // Do not publish anything until initial pose is received in localization mode
  if (this->localization_mode_ && !this->initial_pose_received_) {
    return;
  }
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  // nav_msgs::msg::Path
  //! accumulating pose in the Path msg causes computation bottleneck over time for callbackPointcloud
...
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  pcl::PointCloud<PointType>::Ptr deskewed_scan_world = std::make_shared<pcl::PointCloud<PointType>>();
  pcl::transformPointCloud (*published_cloud, *deskewed_scan_world, T_cloud);

  // Then transform from world to current baselink frame
  // T = current robot pose in world frame
  // We want points in baselink frame, so apply T.inverse()
  Eigen::Matrix4f T_world_to_base = this->T.inverse();
  pcl::transformPointCloud (*deskewed_scan_world, *deskewed_scan_t_, T_world_to_base);

  // published deskewed cloud in baselink frame
  ...
  deskewed_ros.header.frame_id = this->baselink_frame;  // Changed from odom_frame to baselink_frame
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  // 스캔 헤더 타임스탬프와 첫 번째 포인트의 타임스탬프를 일치시켜주기 위함
  double offset = 0.0;
  ...
  // 왜 median 으로 설정하냐면, GICP 결과가 기본적으로 포인트 클라우드의 무게중심을 기준으로 추정되므로,
  // 따라서 무게 중심과 가장 가까운 포인트로 볼 수 있는 scan 의 중심점에서 GICP 의 추정 pose가 가장 정확하다고 볼 수 있기 때문인것으로 생각됨.
  ...
  // don't process scans until IMU data is present
  if (!this->first_valid_scan) {
    // 조건이 이렇게 되는 이유: IMU 적분 구간이 this->scan_stamp 이전 데이터 중 가장 최근의 IMU 데이터 부터 시작이기 때문
    if (this->imu_buffer.empty() || this->scan_stamp <= this->imu_buffer.back().stamp) {
      return;
    }
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> frames;
  // 논문이랑 다른 부분: 논문에서는 integrateImu 할 때 직전 scan 으로 최적화된 pose, velocity (this->state) 을 쓰지만,
  // 여기서는 p, q 는 직전 scan 의 GICP 결과를 쓰고, velocity 는 observer 로 업데이트 된 state 의 veocity 를 씀
  // p, q 도 this->state.prev_p, q 를 쓰면 되는거 아닌가 싶은데, 이 코드의 의도는 기하학적으로 가장 정확한 관측결과인
  // GICP 의 결과를 deskew 에 쓰는게 타당하다는것을 의미, state 의 p, q 는 imu prior 와 GICP 결과가 융합된 결과이므로
  // 기하학적으로 정확한 (scan 과 submap 이 가장 잘 정렬되는) p, q 가 아님. 따라서, GICP 의 결과물인
  // lidarPose 를 쓰는 것임
  frames = this->integrateImu(this->prev_scan_stamp, this->lidarPose.q, this->lidarPose.p,
...
// multi-threading point correction
#pragma omp parallel for num_threads(this->num_threads_)
  for (int i = 0; i < timestamps.size(); i++) {

    // frame is a base pose in world frame: world->base, thus T is world->lidar
    Eigen::Matrix4f T = frames[i] * this->extrinsics.baselink2lidar_T;

    // transform point to world frame
    // each pt is in the lidar frame, so we transform it to the world frame by multiplying T
...
void dlio::OdomNode::initializeInputTarget() {
  ...
  // Set the first scan as a keyframe in odometry mode
  ...
  if (this->gicp_implementation_ == "small_gicp") {
    this->small_keyframe_normals.push_back(std::make_shared<const std::vector<Eigen::Matrix4d>>(this->small_source_normals)); //! small_gicp
  } else {
    this->nano_keyframe_normals.push_back(this->ngicp.getSourceCovariances()); // nano_gicp
  }  
...
void dlio::OdomNode::setInputSource() {
  //! small_gicp
  // GICP 동작을 위해 현재 스캔을 소스로 설정
  // setInputSource 호출되면, point cloud 등록 및 kd-tree 구축, covariance 계산이 이루어짐
  if (this->gicp_implementation_ == "small_gicp") {
    this->small_source_normals.resize(this->current_scan->size());
    small_gicp::PointCloudProxy<PointType> proxy(*this->current_scan, this->small_source_normals);
    small_gicp::estimate_covariances_omp(
        proxy, 
        this->gicp_k_correspondences_, 
        this->num_threads_
    );
  } else {
    this->ngicp.setInputSource(this->current_scan);
    this->ngicp.calculateSourceCovariances();
  }
...
void dlio::OdomNode::initializeDLIO() {
  // In localization mode, wait for the initial pose
  if (this->localization_mode_ && !this->initial_pose_received_) {
    return;
  }
  // Wait for IMU
...
// New function to load a map from a .pcd or .ply file
void dlio::OdomNode::loadMap() {
  if (this->map_path_.empty()) {
    RCLCPP_ERROR(this->get_logger(), "Map path is empty. Please provide a map file.");
    return;
  }
  ...
  // Get file extension
  std::string extension = this->map_path_.substr(this->map_path_.find_last_of(".") + 1);

  if (extension == "pcd") {
    if (pcl::io::loadPCDFile<PointType>(this->map_path_, *this->loaded_map_pcd_) == -1) {
      RCLCPP_ERROR(this->get_logger(), "Couldn't read file %s", this->map_path_.c_str());
      return;
    }
  } else if (extension == "ply") {
    if (pcl::io::loadPLYFile<PointType>(this->map_path_, *this->loaded_map_pcd_) == -1) {
      RCLCPP_ERROR(this->get_logger(), "Couldn't read file %s", this->map_path_.c_str());
      return;
    }
  } else {
    RCLCPP_ERROR(this->get_logger(), "Unsupported map file format: %s. Please use .pcd or .ply", extension.c_str());
    return;
  }

  RCLCPP_INFO(this->get_logger(), "Map loaded with %ld points.", this->loaded_map_pcd_->points.size());


  // Voxelize the map
  if (this->vf_use_) {
    RCLCPP_INFO(this->get_logger(), "Applying voxel grid filter with leaf size: %f", this->vf_res_);
    this->voxel.setInputCloud(this->loaded_map_pcd_);
    this->voxel.filter(*this->loaded_map_pcd_);
    RCLCPP_INFO(this->get_logger(), "Map has %ld points after voxel filtering.", this->loaded_map_pcd_->points.size());
  }

  if (this->loaded_map_pcd_->points.empty()) {
    RCLCPP_FATAL(this->get_logger(), "Map is empty after preprocessing! Check voxel filter leaf size (`odom/preprocessing/voxelFilter/res`). Aborting.");
    return;
  }
  
  RCLCPP_INFO(this->get_logger(), "Map loaded successfully. Building KD-Tree for GICP target...");
  auto start_time = this->now();

  //! small_gicp
  if (this->gicp_implementation_ == "small_gicp") {
    this->small_submap_kdtree = std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointType>>>(
        this->loaded_map_pcd_, 
        small_gicp::KdTreeBuilderOMP(this->num_threads_)
    );
    this->small_submap_normals.resize(this->loaded_map_pcd_->size());

    small_gicp::PointCloudProxy<PointType> proxy(*this->loaded_map_pcd_, this->small_submap_normals);

    small_gicp::estimate_covariances_omp(
        proxy, 
        *this->small_submap_kdtree,
        this->gicp_k_correspondences_, 
        this->num_threads_
    );
    this->submap_cloud = this->loaded_map_pcd_;
  } else {
    // nano_gicp
    this->ngicp.setInputTarget(this->loaded_map_pcd_);
    this->ngicp.calculateTargetCovariances();
  }

  auto end_time = this->now();
  // --- 추가: GICP Target 설정 완료 로그 및 소요 시간 출력 ---
  RCLCPP_INFO(this->get_logger(), "KD-Tree built in %.4f seconds.", (end_time - start_time).seconds());

  // Publish the loaded map
  sensor_msgs::msg::PointCloud2 map_ros;
  pcl::toROSMsg(*this->loaded_map_pcd_, map_ros);
  map_ros.header.stamp = this->now();
  map_ros.header.frame_id = this->odom_frame;
  this->map_pub_->publish(map_ros);
}

// Internal function to set initial pose (shared logic for auto and manual initialization)
// This follows the exact same logic as callbackInitialPose
void dlio::OdomNode::setInitialPoseInternal(double x, double y, double z, double qx, double qy, double qz, double qw) {
    // 1. Get pose from the initial pose message
    this->state.p[0] = x;
    this->state.p[1] = y;
    this->state.p[2] = z;
...
// New callback for receiving the initial pose
void dlio::OdomNode::callbackInitialPose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr pose) {
    if (!this->localization_mode_) {
        return;
    }
  ...
  RCLCPP_INFO(this->get_logger(), "Initial pose received. Starting localization.");
}

void dlio::OdomNode::callbackPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
  // In localization mode, wait for the initial pose before processing point clouds
  if (this->localization_mode_ && !this->initial_pose_received_) {
    return;
  }

  // main_loop_running stands for that the lidar callback is still being processed
  ...
  // Convert incoming scan into DLIO format (and crop box filtering)
  this->getScanFromROS(pc);

  // Preprocess points (deskewing and voxel filtering)
  ...
  // 이 기능은 입력 스캔의 spaciousness (포인트와 센서 사이의 평균적 거리) 와 density (포인트간 거리) 값을 기준으로 keyframe 생성 간격 및
  // GICP 의 max_correspondence_distance 를 조절하는 기능임
  // 복도나 좁은 환경에서는 sp 는 작아질 것이고, density 는 커지게됨
  // 그런데 문제는 동적 장애물에 의해서도 sp 와 density 가 영향을 받기 때문에 의도했던 동작이 안될 수 도 있음
  ...
  // ==========================================================================================
      ...
      if (this->keyframes.size() == 0) {
        this->initializeInputTarget(); // add first keyframe
      ...
      // keyframe 등록도 여기서 되는데, 등록할 때 deskewed scan 을 등록함
  ...
  // multi-threading publishToROS causes "free(): invalid next size (normal)"
  ...
  ++this->global_comp_frame_count;
  this->global_comp_time_sum += this->comp_times.back();
  this->global_avg_comp_time = this->global_comp_time_sum / this->global_comp_frame_count;
  ...
  if (!this->imu_calibrated) {
    ...
    if ((imu_stamp_secs - this->first_imu_stamp) < this->imu_calib_time_) {
      ...
      if (this->gravity_align_) {
        ...
        Eigen::Vector3f body_x_tilted = grav_q * Eigen::Vector3f::UnitX();
        
        // XY 평면에 투영하여 현재 Yaw 계산 (atan2는 -PI ~ +PI 범위를 정확히 반환함)
        double yaw_current_grav = std::atan2(body_x_tilted.y(), body_x_tilted.x());

        double yaw_target = 0.0;

        if (this->localization_mode_ && this->initial_pose_received_) {
            Eigen::Vector3f target_x = this->state.q * Eigen::Vector3f::UnitX();
            yaw_target = std::atan2(target_x.y(), target_x.x());
        } else {
            // Odometry 모드: 0.0 (정면)
            yaw_target = 0.0;
        }

        double yaw_diff = yaw_target - yaw_current_grav;

        if (yaw_diff > M_PI) yaw_diff -= 2 * M_PI;
        if (yaw_diff < -M_PI) yaw_diff += 2 * M_PI;

        Eigen::Quaternionf aligned_q = Eigen::AngleAxisf(yaw_diff, Eigen::Vector3f::UnitZ()) * grav_q;

        // set gravity aligned orientation
        this->state.q = aligned_q;
      ...
      this->imu_calibrated = true;
      // for the first imu measurement, set the previous imu stamp to the current imu stamp
...
void dlio::OdomNode::getNextPose() {
  ...
  if (this->new_submap_is_ready && this->submap_hasChanged) {
    // small_gicp는 align 호출 시 트리를 인자로 받으므로 별도의 등록 과정 불필요

    // nano_gicp
    if (this->gicp_implementation_ == "nano_gicp") {
      // Set the current global submap as the target cloud
      this->ngicp.registerInputTarget(this->submap_cloud);

      // Set submap kdtree
      this->ngicp.target_kdtree_ = this->nano_submap_kdtree;

      // Set target cloud's normals as submap normals
      this->ngicp.setTargetCovariances(this->nano_submap_normals);
  ...
  if (this->gicp_implementation_ == "small_gicp") {
    // proxy generation for small_gicp
    small_gicp::PointCloudProxy<PointType> target_proxy(*this->submap_cloud, this->small_submap_normals);
    small_gicp::PointCloudProxy<PointType> source_proxy(*this->current_scan, this->small_source_normals);

    // use identity as prior since source is already in the world frame due to deskewing
    Eigen::Isometry3d gicp_T_temp;
    if (this->sgicp_use_robust_) {
      auto result = this->cauchy_sgicp.align(target_proxy, source_proxy, *this->small_submap_kdtree, Eigen::Isometry3d::Identity());
      gicp_T_temp = result.T_target_source;
    } else {
      auto result = this->sgicp.align(target_proxy, source_proxy, *this->small_submap_kdtree, Eigen::Isometry3d::Identity());
      gicp_T_temp = result.T_target_source;
    }

    // Get final transformation in global frame
    this->T_corr = gicp_T_temp.cast<float>().matrix(); // "correction" transformation
  } else {
    // nano_gicp
    pcl::PointCloud<PointType>::Ptr aligned = std::make_shared<pcl::PointCloud<PointType>>();
    this->ngicp.align(*aligned);

    // Get final transformation in global frame
    this->T_corr = this->ngicp.getFinalTransformation(); // "correction" transformation
  }
...
bool dlio::OdomNode::imuMeasFromTimeRange(double start_time, double end_time,
  ...
  if (this->imu_buffer.empty() || this->imu_buffer.front().stamp < end_time) {
    ...
    // there should be an imu measurement that is later than the end_time which is the timestamp of the last point of the scan
  ...
  // .begin() is the latest imu measurement iterator since we push_front the imu measurements
  ...
  // at this point, imu_it is either the end of the buffer that has later timestamp than the start_time
  // or the iterator that has the earlier timestamp than the start_time
...
dlio::OdomNode::integrateImu(double start_time, Eigen::Quaternionf q_init, Eigen::Vector3f p_init,
  ...
  // Acceleration at first IMU sample w.r.t the world frame
  // q_init 은 world 기준 lidar 자세를 나타내는 쿼터니언
  Eigen::Vector3f a1 = q_init._transformVector(f1.lin_accel);
  // lio 가 가정하는 world 프레임은 local tangent frame 임, 즉 세계가 평평하다고 가정하는 것
  // 따라서, 중력벡터가 로봇의 position 에 관계없이 고정이기 때문에 z축만 고려해서 중력 보상을 하는 것
  a1[2] -= this->gravity_;
...
void dlio::OdomNode::propagateState() {
  ...
  // Note: imu_meas.lin_accel still includes gravity, so we remove it in world frame (Z-axis)
  world_accel = qhat._transformVector(this->imu_meas.lin_accel);

  // Accel propogation - remove gravity in world frame (Z-axis)
...
void dlio::OdomNode::computeDensity() {
  ...
  if (!this->geo.first_opt_done) {
    ...
    // nano_gicp 와 달리 small_gicp 는 density 계산이 내부적으로 수행되지 않음.
    //Todo : small_gicp 에서 density 계산 기능 추가 고려
    density = 0.; //! small_gicp
    if (this->gicp_implementation_ == "small_gicp") {
      density = 0.;
    } else {
      density = this->ngicp.source_density_; // nano_gicp
    }
  }
...
void dlio::OdomNode::setAdaptiveParams() {
  ...
  this->gicp_max_corr_dist_ = den; //! small_gicp
  this->ngicp.setMaxCorrespondenceDistance(den); // nano_gicp
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  // for loop 돌면서 모든 키프레임 다 탐색을 해야하나 꼭? 키프레임 수가 몇개 안되니까 이렇게 한건가?
  std::unique_lock<decltype(this->keyframes_mutex)> lock(this->keyframes_mutex);
  std::vector<float> ds;
  ...
  // 단순히 현재 포즈 근처의 keyframe 만 선택하지 않고, 넓은 범위로 퍼져있는 keyframe 도 submap 에 포함하기 위해
  // convex hull 과 concave hull 을 사용
  // submap 에 지역적 특징 뿐 아니라 전역적 특징도 포함시켜서 drift 를 줄이기 위함으로 보임
  ...
  // knn, convex, concave hull 에서 뽑힌 keyframe 중 중복된것 제거
  ...
  // 이전과 다른 경우에만 submap 을 재구성 -> point cloud 변환 및 kd-tree 재생성 같은
  // 비교적 무거운 작업들을 하기 때문에 필요한 경우에만 재구성하고
  // 또, point cloud callback 이 바쁜 경우(중요한 계산 중) submap 재구성을 잠시 멈춰서 cpu 리소스 경쟁 안하도록 함
  if (this->submap_kf_idx_curr != this->submap_kf_idx_prev){
    ...
    std::vector<Eigen::Matrix4d> small_submap_normals_; //! small_gicp
    std::shared_ptr<nano_gicp::CovarianceList> nano_submap_normals_ (std::make_shared<nano_gicp::CovarianceList>()); // nano_gicp

    for (auto k : this->submap_kf_idx_curr) {

      // create current submap cloud
      // 키프레임은 이미 월드 프레임 기준 이므로, 그냥 합치기만 하면 됨
      // 아래는 pcl 기준 병합 문법, submap_cloud 벡터의 크기를 오른쪽 클라우드 포인트 수 만큼 늘리고 거기에 데이터 복사
      // 메모리 I/O 를 거치는 복사이므로, 병목발생할 수 있는 부분임
      lock.lock();
      *submap_cloud_ += *this->keyframes[k].second;
      ...
      if (this->gicp_implementation_ == "small_gicp") {
        const auto& covs = *(this->small_keyframe_normals[k]); //! small_gicp
        small_submap_normals_.insert(small_submap_normals_.end(), covs.begin(), covs.end()); //! small_gicp
      } else {
        // nano_gicp
        nano_submap_normals_->insert( std::end(*nano_submap_normals_),
            std::begin(*(this->nano_keyframe_normals[k])), std::end(*(this->nano_keyframe_normals[k])) );
      }
    }
    ...
    if (this->gicp_implementation_ == "small_gicp") {
      //! small_gicp
      auto submap_kdtree_ = std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointType>>>(
          submap_cloud_, 
          small_gicp::KdTreeBuilderOMP(this->num_threads_)
      );
      this->submap_cloud = submap_cloud_;
      this->small_submap_normals = std::move(small_submap_normals_);
      this->small_submap_kdtree = submap_kdtree_;
    } else {
      // nano_gicp
      this->submap_cloud = submap_cloud_;
      this->nano_submap_normals = nano_submap_normals_;  
      this->ngicp_temp.setInputTarget(this->submap_cloud);
      this->nano_submap_kdtree = this->ngicp_temp.target_kdtree_;
    }
    // pause again before updating pointers to prevent race conditions
    // this->pauseSubmapBuildIfNeeded();
...
void dlio::OdomNode::buildKeyframesAndSubmap(State vehicle_state) {
  ...
  for (int i = this->num_processed_keyframes; i < this->keyframes.size(); i++) {
    ...
    //! small_gicp
    std::shared_ptr<const std::vector<Eigen::Matrix4d>> small_raw_covariances = this->small_keyframe_normals[i];
    // nano_gicp
    std::shared_ptr<const nano_gicp::CovarianceList> nano_raw_covariances = this->nano_keyframe_normals[i];
    ...
    if (this->gicp_implementation_ == "small_gicp") {
      //! small_gicp 로직
      std::shared_ptr<const std::vector<Eigen::Matrix4d>> raw_covariances = this->small_keyframe_normals[i];
      
      std::shared_ptr<std::vector<Eigen::Matrix4d>> transformed_covariances = 
          std::make_shared<std::vector<Eigen::Matrix4d>>(raw_covariances->size());

      // small_gicp용 transform
      ...
      lock.lock();
      this->small_keyframe_normals[i] = transformed_covariances;

    } else {
      // nano_gicp 로직
      std::shared_ptr<const nano_gicp::CovarianceList> raw_covariances = this->nano_keyframe_normals[i];
      ...
      // nano_gicp용 transform
      std::transform(raw_covariances->begin(), raw_covariances->end(), transformed_covariances->begin(),
                   [&Td](Eigen::Matrix4d cov) { return Td * cov * Td.transpose(); });

      lock.lock();
      this->nano_keyframe_normals[i] = transformed_covariances;
    }
...
void dlio::OdomNode::debug() {
  ...
  // Rate limit debug output to 1 Hz
  auto debug_now = std::chrono::steady_clock::now();
  auto time_since_last = std::chrono::duration_cast<std::chrono::milliseconds>(debug_now - this->last_debug_time).count();
  if (time_since_last < 1000) {
    return;  // Skip this debug output
  }
  this->last_debug_time = debug_now;
  ...
  std::cout << "| Computation Time :: "
    ...
    << "     |" << std::endl;
  std::cout << "| " << std::left << std::setfill(' ') << std::setw(66)
    << "Global Avg. Comp. Time :: " + to_string_with_precision(this->global_avg_comp_time * 1000., 2) + " ms"
    << "|" << std::endl;
