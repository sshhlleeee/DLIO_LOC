...
dlio::OdomNode::OdomNode() : Node("dlio_odom_node") {
  ...
  this->deskew_size = 0;
  // initialize initial pose received flag
  ...
  // New subscriber for the initial pose
  ...
  // add map publisher
  this->map_pub_      = this->create_publisher<sensor_msgs::msg::PointCloud2>("map", 1);
  ...
  // New PointCloud for the loaded map
...
void dlio::OdomNode::initializeInputTarget() {
  ...
  // In localization mode, the loaded map is already the target
}
...
void dlio::OdomNode::loadMap() {
  ...
  // --- 추가: GICP Target으로 설정하기 전, 최종 포인트 수 확인 ---
...
void dlio::OdomNode::callbackInitialPose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr pose) {
    ...
    this->state.p[0] = pose->pose.pose.position.x;
    this->state.p[1] = pose->pose.pose.position.y;
    this->state.p[2] = pose->pose.pose.position.z;

    this->state.q.w() = pose->pose.pose.orientation.w;
    this->state.q.x() = pose->pose.pose.orientation.x;
    this->state.q.y() = pose->pose.pose.orientation.y;
    this->state.q.z() = pose->pose.pose.orientation.z;
